              ...
              <tbody>
                ...
                {starterOrder.map((pos) => (
                  <tr key={pos} className="text-center">
                    <td>
                      {team1Map[pos] ? (
                        <Link to={`/player/${team1Map[pos].name}`}>
                          {team1Map[pos].name}
                        </Link>
                      ) : (
                        "-"
                    ...
                    <td>
                      {team2Map[pos] ? (
                        <Link to={`/player/${team2Map[pos].name}`}>
                          {team2Map[pos].name}
                        </Link>
                      ) : (
                        "-"
                ...
                {(() => {
                  ...
                  const team1Class =
                    isCompleted && team1Total > team2Total ? "winner" : "bold";
                  const team2Class =
                    isCompleted && team2Total > team1Total ? "winner" : "bold";
                ...
                {(() => {
                  const getFlexBenchPlayers = (roster) => {
                    return roster
                      .filter((entry) => {
                        const pos = (
                          entry.position || entry.player?.position || ""
                        ).toLowerCase();
                  ...
                  for (let i = 0; i < maxBenchCount; i++) {
                    ...
                    const posLabel =
                      team1Player?.pos || team2Player?.pos || "";
                    ...
                    rows.push(
                      <tr key={`flexbench-row-${i}`} className="text-center">
                        ...
                        <td>
                          {team1Player
                            ? getPoints(team1Player.player, match.week)
                            : "-"}
                        </td>
                        <td>{posDisplay}</td>
                        <td>
                          {team2Player
                            ? getPoints(team2Player.player, match.week)
                            : "-"}
                        </td>
                        <td>
                          {team2Player ? (
